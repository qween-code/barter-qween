 ...
 */
async function findPotentialMatches(sourceItemId: string, sourceItemData: any) {
 ...
 */
async function calculateMatchScore(sourceItem: any, targetItem: any) {
  let categoryScore = 0;
  let priceScore = 0;
  let locationScore = 0;
  let trustScore = 0;
  ...
  let quality = 'poor';
  if (overallScore >= 80) quality = 'excellent';
  else if (overallScore >= 65) quality = 'good';
  else if (overallScore >= 50) quality = 'fair';
  
  // Generate match reasons
  const matchReasons = [];
  if (categoryScore >= 80) matchReasons.push('Same category');
  if (priceScore >= 70) matchReasons.push('Similar price range');
  if (locationScore >= 80) matchReasons.push('Close location');
  if (trustScore >= 80) matchReasons.push('High trust users');
  if (conditionScore >= 80) matchReasons.push('Matching condition');
  
  // Generate concerns
  const concerns = [];
  if (priceScore < 50) concerns.push('Significant price difference');
  if (locationScore < 50) concerns.push('Far distance for meetup');
  if (trustScore < 50) concerns.push('Lower trust scores');
 ...
 */
async function createBarterMatch(
  sourceItemId: string, 
  targetItemId: string, 
  sourceItem: any, 
  targetItem: any, 
 ...
 */
async function deleteExistingMatches(itemId: string) {
 ...
 */
function calculateCashDifferential(sourcePrice: number, targetPrice: number): number | null {
