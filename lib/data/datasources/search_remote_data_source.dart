...
class SearchRemoteDataSource {
  ...
  Stream<List<ItemModel>> searchItems({
    ...
    if (filters.statuses != null && filters.statuses!.isNotEmpty) {
      final statusStrings = filters.statuses!
    ...
    final isDescending = filters.sortDirection == SortDirection.descending;
    ...
    return firestoreQuery.snapshots().map((snapshot) {
      return snapshot.docs.map((doc) {
        return ItemModel.fromJson({...doc.data(), 'id': doc.id});
      }).toList();
    });
  }
  ...
  Future<List<String>> getPopularSearches({
    ...
    Query<Map<String, dynamic>> query = firestore.collection('searchAnalytics');
