...
import '../../../core/widgets/safe_card.dart';
...
class _ItemCardWidgetState extends State<ItemCardWidget> {
          ...
          child: LayoutBuilder(
            builder: (context, constraints) {
              final cardWidth = constraints.maxWidth.isFinite
                  ...
                  : 200.0;
              final imageExtent = _calculateImageExtent(cardWidth, constraints);
              final maxCardHeight = _calculateMaxCardHeight(constraints);
              ...
              return Column(
                ...
                children: [
                  SizedBox(
                    height: imageExtent,
                    child: _buildImageArea(isNew, cardWidth),
                  ),
                  Expanded(
                    child: _buildBottomSection(context, cardWidth),
                  ),
                ],
              );
            },
  ...
  Widget _buildImageArea(bool isNew, double cardWidth) {
    return Stack(
      fit: StackFit.expand,
      children: [
        Positioned.fill(
  ...
  Widget _buildBottomSection(BuildContext context, double cardWidth) {
    final hasBrand =
  ...
  /// Calculate optimal image extent based on constraints
  double _calculateImageExtent(double cardWidth, BoxConstraints constraints) {
    // Image should be square aspect ratio
    final calculatedExtent = cardWidth * 0.95; // 95% of card width
    
    // Ensure reasonable bounds (minimum 120, max 280)
    return calculatedExtent.clamp(120.0, 280.0);
  }

  /// Calculate maximum card height to prevent overflow
  double _calculateMaxCardHeight(BoxConstraints constraints) {
    final screenHeight = MediaQuery.of(context).size.height;
    final cardWidth = constraints.maxWidth.isFinite ? constraints.maxWidth : 200.0;
    
    // Calculate based on content: image + bottom section
    final imageHeight = _calculateImageExtent(cardWidth, constraints);
    final bottomSectionHeight = 140.0; // Estimated content height
    final totalEstimated = imageHeight + bottomSectionHeight;
    
    // Ensure it doesn't exceed screen bounds
    final maxAllowed = screenHeight * 0.65;
    
    return totalEstimated.clamp(260.0, maxAllowed);
  }
}
