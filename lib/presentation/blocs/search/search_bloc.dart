  ...
  Future<void> _onSearchWithFilters(
    ...
    try {
      ...
      result.fold((failure) => emit(SearchError(message: failure.message)), (
        ...
        // Category filter
        if (event.filters.categories != null &&
            event.filters.categories!.isNotEmpty) {
          filteredItems = filteredItems
              .where(
                (item) => event.filters.categories!.contains(item.category),
              )
              .toList();
        }
        ...
        if (event.filters.minPrice != null || event.filters.maxPrice != null) {
          filteredItems = filteredItems.where((item) {
            ...
            final minOk =
                event.filters.minPrice == null ||
                price >= event.filters.minPrice!;
            final maxOk =
                event.filters.maxPrice == null ||
                price <= event.filters.maxPrice!;
            return minOk && maxOk;
          }).toList();
        }

        // Condition filter
        if (event.filters.conditions != null &&
            event.filters.conditions!.isNotEmpty) {
          filteredItems = filteredItems
              .where(
                (item) => event.filters.conditions!.contains(item.condition),
              )
              .toList();
        }
