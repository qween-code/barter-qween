...
class BarterMatchingService {
  ...
  Future<BarterMatchEntity> calculateEnhancedMatch({
    ...
    final mlData = _prepareMLData(scores, sourceItem, targetItem, sourceUser, targetUser);
    ...
    final cashDifferential = _calculateCashDifferential(sourceItem.price, targetItem.price);
    ...
    return BarterMatchEntity(
      ...
      locationDescription: distanceKm != null ? 
        '${distanceKm.toStringAsFixed(1)} km away' : null,
      
      // Barter conditions compatibility
      conditionsCompatible: insights['conditionsCompatible'] ?? true,
      compatibilityNote: insights['compatibilityNote'] ?? 'Barter conditions compatible',
  ...
  List<BarterMatchEntity> sortMatches({
    ...
    switch (criteria) {
      case BarterSortCriteria.bestMatch:
        sortedMatches.sort((a, b) => ascending 
  ...
  Future<Map<String, double>> _calculateDetailedScores({
    ...
    scores['compatibility'] = _calculateCompatibilityScore(sourceItem, targetItem);
    ...
    scores['userPreference'] = _calculateUserPreferenceScore(sourceUser, targetItem);
  ...
  bool _isBasicCompatible(ItemEntity sourceItem, ItemEntity targetItem) {
    ...
    // Status check
    if (sourceItem.status != 'available' || targetItem.status != 'available') {
      return false;
    }
    
    // Barter condition check
    if (sourceItem.barterCondition == null || targetItem.barterCondition == null) {
      return false;
    }
  ...
  double _calculatePriceScore(double? sourcePrice, double? targetPrice) {
    if (sourcePrice == null || targetPrice == null || sourcePrice == 0 || targetPrice == 0) {
      return 50.0; // Neutral score
    }
    
    final priceRatio = min(sourcePrice, targetPrice) / max(sourcePrice, targetPrice);
    final priceDiff = (sourcePrice - targetPrice).abs() / max(sourcePrice, targetPrice);
  ...
  Future<double> _calculateLocationScore(ItemEntity sourceItem, ItemEntity targetItem) async {
    if (sourceItem.latitude == null || sourceItem.longitude == null ||
        targetItem.latitude == null || targetItem.longitude == null) {
  ...
  double _calculateConditionScore(ItemEntity sourceItem, ItemEntity targetItem) {
  ...
  double _calculateCompatibilityScore(ItemEntity sourceItem, ItemEntity targetItem) {
  ...
  double _calculateSeasonalityScore(ItemEntity sourceItem, ItemEntity targetItem) {
  ...
  Map<String, dynamic> _generateMatchInsights(
    ...
    return {
      ...
      'marketDemand': scores['demand']! > 70 ? 'High demand item' : 'Standard demand',
      'seasonalMatch': scores['seasonality']! > 70 ? 'Seasonal match' : 'Regular match',
      'userPreferenceMatch': scores['userPreference']! > 70 ? 'User preference match' : 'Neutral',
    };
  }
  ...
  double _calculateFinalScore(Map<String, double> scores, Map<String, dynamic> insights) {
  ...
  MatchQuality _determineMatchQuality(double score, Map<String, dynamic> insights) {
  ...
  List<String> _generateMatchReasons(Map<String, double> scores, Map<String, dynamic> insights) {
    final reasons = <String>[];
    
    if (scores['category']! >= 80) reasons.add('Same category');
    if (scores['price']! >= 70) reasons.add('Similar price range');
    if (scores['location']! >= 80) reasons.add('Close location');
    if (scores['trust']! >= 80) reasons.add('High trust users');
    if (scores['condition']! >= 80) reasons.add('Matching condition');
    if (scores['compatibility']! >= 70) reasons.add('Barter conditions compatible');
  ...
  /// Generate concerns
  List<String> _generateConcerns(Map<String, double> scores, Map<String, dynamic> insights) {
    final concerns = <String>[];
    
    if (scores['price']! < 50) concerns.add('Significant price difference');
    if (scores['location']! < 50) concerns.add('Far distance for meetup');
    if (scores['trust']! < 50) concerns.add('Lower trust scores');
    if (scores['compatibility']! < 50) concerns.add('Barter conditions not well matched');
  ...
  Future<double?> _calculateDistance(ItemEntity sourceItem, ItemEntity targetItem) async {
    if (sourceItem.latitude == null || sourceItem.longitude == null ||
        targetItem.latitude == null || targetItem.longitude == null) {
      return null;
    }
  ...
  CashDirection _determineCashDirection(double? sourcePrice, double? targetPrice) {
    if (sourcePrice == null || targetPrice == null) return CashDirection.none;
    
    if (sourcePrice > targetPrice) return CashDirection.fromInitiator;
    if (targetPrice > sourcePrice) return CashDirection.toInitiator;
    return CashDirection.none;
  }
  ...
  Future<void> _logMLTrainingData(Map<String, dynamic> mlData, double finalScore) async {
    try {
